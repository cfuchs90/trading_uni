...
\gls{bah} strategy, which only consists of 2 trades. However just looking at the net trading \gls{pl} it becomes immediately clear the the
...
possible to decide whether the \gls{dcs} can be considered sucessfulle in contrast to the \gls{bah} strategy.
...
The above sections showed the reader that a quantitative trading strategy cannot be implemented without taking further risks, which confirms the efficient m
